...
class RoleController extends AdminController{
    ...
    function distribute($role_id){
        if(!empty($_POST)){
            ...
            $pauth_info = D('Auth')->where('auth_level=0 and deleted=0')->select(); //父级权限
            $sauth_info = D('Auth')->where('auth_level=1 and deleted=0')->select(); //次父级权限
            $tauth_info = D('Auth')->where('auth_level=2 and deleted=0')->select(); //次次父级权限
