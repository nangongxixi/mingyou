...
var getFilters = function() {
  ...
  var filters = {
    ...
    log_reported_time: {
      ...
      error: $.idealforms.errors.log_reported_time
    },
      ...
      regex: function( input, value ) {
        ...
        isDate = function( date, format ) {
          ...
          for ( var i = 0, len = format.length; i < len; i++ ) {            
            ...
            if ( /h/.test( format[i]) ) h = date[i]
            if ( /m/.test( format[i]) ) m = date[i]
            if ( /s/.test( format[i]) ) s = date[i]
          }
          return (
            m > 0 && m < 13 && y && y.length === 4 &&  d > 0 && d <= ( new Date( y, m, 0 ) ).getDate()
          )
        }
