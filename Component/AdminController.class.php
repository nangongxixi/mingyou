...
class AdminController extends Controller {
    ...
    function __construct() {
        ...
        $sql = "select role_auth_ac from yw_manager a join yw_role b on a.mg_role_id=b.role_id where a.mg_id=" . $_SESSION['mg_id'];
        $auth_ac = D()->query($sql);
        ...
        $allow_ac = array('Index-left', 'Index-right', 'Index-head', 'Index-index', 'Manager-login', 'Manager-logout', 'Server-checkUser', 'Manager-verifyImg');
        if (!in_array($now_ac, $allow_ac) && $_SESSION['mg_id'] != 1 && strpos($auth_ac, $now_ac) === false) {
            //echo '<script>alert("您没有权限访问！");window.location.href="../Index/index.html";</script>';
